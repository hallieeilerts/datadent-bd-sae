...
ea <- st_read("./data/BD_2014_DHS_08202025_155_120781/BDGE71FL", layer = "BDGE71FL")
...
# There are a few that were NA in the merge
nrow(subset(points_with_district, is.na(ADM2_EN))) # 3
# subset(points_with_district, is.na(ADM2_EN))
# subset(ea_crs, DHSCLUST %in% c(17, 21, 544))
# # Fill in these two manually based on nearby clusters
# subset(points_with_district, DHSCLUST %in% c(17, 21))
# subset(points_with_district, DHSCLUST %in% c(16, 17, 18, 19, 20, 21, 22))
points_with_district$ADM1_EN[is.na(points_with_district$ADM2_EN) & 
                               points_with_district$DHSCLUST %in% c(17, 21)] <- "Barisal"
points_with_district$ADM2_EN[is.na(points_with_district$ADM2_EN) & 
                               points_with_district$DHSCLUST %in% c(17)] <- "Barisal"
points_with_district$ADM2_EN[is.na(points_with_district$ADM2_EN) & 
                               points_with_district$DHSCLUST %in% c(21)] <- "Bhola"
# Cluster 544 has 0 for lat and long. drop.
points_with_district <- subset(points_with_district, !is.na(ADM2_EN))
...
write.csv(df, "./gen/prepare-shp/output/cluster-locations-2014.csv", row.names = FALSE)
