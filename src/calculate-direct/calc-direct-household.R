...
dhs_degf <- dhs %>%
  group_by(ADM2_EN) %>%
  summarise(degf = n_distinct(hv001) - 1) # psu/clusters v001? households v002?
...
dhs_degf <- dhs %>%
  summarise(degf = n_distinct(hv001) - 1)
...
write.csv(est_fd, file = "./gen/calculate-direct/temp/direct-household-focusdistricts.csv", row.names = FALSE)
