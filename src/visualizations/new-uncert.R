...
for(i in 1:length(v_var_guidance)){
  ...
  #mymodel <- subset(minerror_adm1_mod , variable == myoutcome)$model
  # FOR GUIDANCE NOTE
  # limit to model chosen using RMSE by covariate grouping in consultation with Maiga, but would like to do minimum error model for ph_wtr_improve
  mymodel <- ind %>%
    filter(variable == myoutcome) %>%
    select(variable, model_covar_grp)  %>%
    mutate(model_covar_grp = ifelse(variable == "ph_wtr_improve", "100-1", model_covar_grp)) %>%
