...
for(i in 1:length(v_var)){
  ...
  # for each model
  v_models <- unique(myinfo$model)
  #v_models <- v_models[!(v_models %in% "100-2")]
  
  datoutcome <- data.frame()
  for(j in 1:length(v_models)){
    ...
    # read rds file that contains names of 2 stan csv files
    # these will specify latest files that were created for this model id (i.e., vers-test)
    ...
    datoutcome <- rbind(df_agg, datoutcome)
  }
  
  dat <- rbind(datoutcome, dat)
...
write.csv(dat, file = "./gen/validation/temp/pred-agg.csv", row.names = FALSE)
